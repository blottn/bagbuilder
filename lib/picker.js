...
const addAvailListener = (domRoot, availabilityRoot) => {
  const obs = new MutationObserver((mutations) => {
    mutations.forEach(({target}) => {
      ...
      let matches = domRoot.querySelectorAll(`.proxy[id="${id}"]`)
        .forEach(e => {
          e.setAttribute("state", avail ? "available" : "unavailable");
        });
    });
  });
          ...
          <div class="tok-proxies" token="${tok}">
            ${
              chars.map(c => tokensFor(c))
                .flat(Infinity)
                .sort((t1, t2) => rate(tok, t1.id) - rate(tok, t2.id))
                .map(({id, edition, team}, idx) => {
                  return `
                  <p class="proxy"
                      state="available"
                      ${(prefs[tok] ?? [tok]).indexOf(id) == -1 ? "" : "preferred"}
                      ${id.includes("_") ? "reminder" : ""}
                      token="${tok}"
                      team="${team}"
                      edition="${edition}"
                      proxy="${id}"
