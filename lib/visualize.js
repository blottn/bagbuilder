...
*/
  ...
  // picked => token -> list<picked>
  const load = ({picked, unavailable}) => {
    Object.entries(picked)
      .forEach(([key, picks]) => {
        picks.forEach((p, i) => {
          document.querySelectorAll(`:nth-child(1 of [token="${key}"] > [proxy="${p}"][state="available"])`)
            .forEach((node, j) => {
              node.setAttribute("state", "inuse");
              if (i == j) {
                node.setAttribute("state", "picked");
              }
            })
        })
      })
  ...
  load(conf)
  return {save, load, togglelistenForUpdate};
}
